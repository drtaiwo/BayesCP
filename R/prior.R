...
bayescp_prior <- function(m01 = 0, m02 = 0,
  ...
  for (nm in names(vals)) {
    .bayescp_assert_scalar(vals[[nm]], nm)
  }
  
  if (kappa01 <= 0 || kappa02 <= 0 || a0 <= 0 || b0 <= 0) {
    stop(
      "Prior concentrations and inverse-gamma hyperparameters must be positive.",
      call. = FALSE
    )
  }
  
  if (!is.null(tau_prob)) {
    if (!is.numeric(tau_prob) ||
        anyNA(tau_prob) ||
        any(!is.finite(tau_prob)) ||
        any(tau_prob < 0) ||
        sum(tau_prob) <= 0) {
      stop(
        "`tau_prob` must contain finite, non-negative values with positive sum.",
        call. = FALSE
      )
    }
